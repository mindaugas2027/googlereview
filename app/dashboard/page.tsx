...
export default function DashboardPage() {
  ...
  const reviewUrl = typeof window !== 'undefined'
    ? `${window.location.origin}/review?business=${encodeURIComponent(user?.id || '')}&google=${encodeURIComponent(business.google_review_url)}&threshold=${business.google_min_rating}`
    : '';
          ...
          {!subscriptionExpired && page === 'qr' && (
            <div className="max-w-3xl">
              ...
              <div className="bg-white border border-[#dadce0] rounded-2xl p-6 shadow-sm grid md:grid-cols-[180px_1fr] gap-7 items-center">
                <div className="bg-white border border-[#dadce0] rounded-xl p-3 w-fit"><QRCodeSVG value={reviewUrl || 'https://getreview.local/review'} size={150} includeMargin /></div>
                <div><h2 className="font-bold text-lg mb-2">Jūsų klientų vertinimo QR kodas</h2><p className="text-sm text-[#5f6368] leading-relaxed mb-4">Klientas nuskenuoja kodą, pasirenka žvaigždutes ir gauna atitinkamą pasiūlymą: patenkinti klientai nukreipiami į Google, o pastabos keliauja jums.</p>{!business.google_review_url && <p className="text-sm text-[#b06000] bg-[#fef7e0] border border-[#f9df96] rounded-lg p-3 mb-4">Pirmiausia pridėkite Google Review URL skiltyje „Vietos“.</p>}<button onClick={() => navigator.clipboard?.writeText(reviewUrl)} className="bg-[#1a73e8] hover:bg-[#1769d1] text-white px-4 py-2.5 rounded-xl text-sm font-semibold">Kopijuoti kliento nuorodą</button></div>
              </div>
            </div>
          )}
          ...
          {page === 'feedback' && (
            <div>
              ...
              <div className="space-y-3">
                {feedbacks.length === 0 && <div className="bg-white border border-dashed border-[#b7bdc4] rounded-2xl p-10 text-center"><span className="h-12 w-12 rounded-2xl bg-[#e8f0fe] text-[#1a73e8] grid place-items-center mx-auto mb-4"><MessageSquare size={22} /></span><h2 className="font-bold text-lg mb-2">Čia matysite klientų balsą</h2><p className="text-sm text-[#5f6368] max-w-sm mx-auto mb-5">Sukurkite QR kodą ir pradėkite rinkti pirmuosius atsiliepimus vienoje vietoje.</p><button onClick={() => setPage('qr')} className="bg-[#1a73e8] hover:bg-[#1769d1] text-white px-4 py-2.5 rounded-xl text-sm font-semibold inline-flex items-center gap-2"><Plus size={16} /> Sukurti QR kodą</button></div>}
                {feedbacks.map((item) => (
