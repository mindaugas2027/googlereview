          ...
          {page === 'billing' && (
            <div className="max-w-3xl">
              ...
              <div className={`rounded-2xl p-6 border ${subscriptionExpired ? 'bg-[#fce8e6] border-[#f5b7b1]' : 'bg-[#e6f4ea] border-[#b7dfc1]'}`}>
                <div className="flex items-center justify-between gap-4 mb-3"><div><p className="text-xs font-bold uppercase tracking-wide text-[#5f6368]">Dabartinis planas</p><h2 className="text-xl font-extrabold mt-1">{plan.name}</h2></div><span className={`text-sm font-bold ${subscriptionExpired ? 'text-[#c5221f]' : 'text-[#137333]'}`}>{subscriptionExpired ? 'Baigėsi' : cancellationScheduled ? `Atšaukta · liko ${trialDaysLeft} d.` : hasPaidSubscription ? `Prenumerata galioja · liko ${trialDaysLeft} d.` : `Bandomasis · liko ${trialDaysLeft} d.`}</span></div>
                <p className="text-sm text-[#3c4043]">{subscriptionExpired ? 'Pasirinkite planą ir vėl gaukite prieigą prie visų savo įmonės duomenų.' : cancellationScheduled ? 'Prenumerata atšaukta, tačiau sistema liks pasiekiama iki nurodyto laikotarpio pabaigos.' : hasPaidSubscription ? 'Jūsų mokama prenumerata galioja.' : 'Jūsų bandomasis laikotarpis galioja. Liko dienų galite matyti čia.'}</p>
                {viewAsId ? (
